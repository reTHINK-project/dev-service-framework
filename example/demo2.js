...
function hypertyDeployed(result) {
  ...
  let videoEl = messageChat.find('.video');
  messageChat.removeClass('hide');

  let connector = components[result.runtimeHypertyURL].hypertyCode.hypertyConnector;

  connector.addEventListener('connector:notification', function(event) {

    let incoming = $('.modal-call');
    let informationHolder = information.find('.information');
    informationHolder.html(event);
...
function openChat(result, video) {
  ...
  let videoEl = messagesChat.find('.video');
  let loginPanel = $('.login-panel');
  ...
  if (video) {
    ...
    var a = components[fromUser].hypertyCode;

    console.log(toUser);

    a.connect(toUser).then(function(controller) {

      controller.addEventListener('stream:added', function(stream, reporter, observer) {
        console.log('Stream: ', stream, reporter, observer);
        videoEl.src = stream;
      });
